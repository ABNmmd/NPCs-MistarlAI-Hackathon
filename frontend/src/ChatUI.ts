...
export class ChatUI {
  ...
  private trustBefore = 0;

  private onCloseCallback: (() => void) | null = null;
  private onPlayerSendCallback: ((msg: string) => void) | null = null;
  private onNPCResponseCallback: ((dialogue: string) => void) | null = null;
  private onConversationEndCallback: ((npcId: string, trustBefore: number, trustAfter: number, emotion: string, memory: any) => void) | null = null;
  ...
  public open(): void {
    ...
    this.trustBefore = this.aiService.getTrustScore();
  ...
  public close(): void {
    ...
    const npcId      = this.aiService.getActiveNpcId();
  ...
  public onPlayerSend(callback: (msg: string) => void): void { this.onPlayerSendCallback = callback; }
  public onNPCResponse(callback: (dialogue: string) => void): void { this.onNPCResponseCallback = callback; }
  public onConversationEnd(
    callback: (npcId: string, trustBefore: number, trustAfter: number, emotion: string, memory: any) => void,
  ): void {
  ...
  private async handleSend(): Promise<void> {
    ...
    this.isSending = true;
    this.onPlayerSendCallback?.(text); // NPC switches to listening
    ...
    try {
      ...
      this.onNPCResponseCallback?.(response); // NPC switches to talking
